...
Mrot = (1/3)*M_connecting_rod + M_balance_weight_on_crankshaft;   % peristrefomeni maza
M_crosshead = 1464;                       % (kg)
M_piston_Mpiston_rod = 1011;               % (kg), piston+piston rod
Mrec = M_crosshead + M_piston_Mpiston_rod + (2/3)*M_connecting_rod ;  % palindromiki maza 3330
B = 0.5;      % bore
...
[time,y] = ode23tb(@main_func,[0 3],initial_conditions,opts);   % epilisi vasikis eksisosis me ode
...
title('100% Load: Rotational Speed - Time')
...
title ('Reciprocating Mass Inertia Torque - Time')
...
xlabel('Angle (rad)')
ylabel('Cylinder Pressure (Pa)')
title('100% Load: Cylinder Pressure - Angle')
...
xlabel('Angle (rad)')
ylabel('Gas Torque (Nm)')
title('100% Load: Gas Torque - Angle')
...
Power_propeller2 = smoothdata(Power_propeller);
...
simeio_P_25 = 1590*10^3;
simeio_n_25 = 62.4;

plot(simeio_n_25,simeio_P_25,'k*')


P_high_margin = (1.111*(62.4/99)^2 - 0.067*(62.4/99))*PMCR;
P_margin = [1590*10^3:vima:P_high_margin];
n_margin = 62.4*ones(size(P_margin));

plot(n_margin,P_margin,'g')

xlabel('Rotational Speed (rpm)')
ylabel('Power (Watt)')
legend(ld([1 7 13 14]),'Loading Diagram','Point A','Propeller Law','Margin','Location','northwest')
